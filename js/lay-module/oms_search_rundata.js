selectBtns_m = function(btn, field, v) {
    layui.oms_r.selectBtns_m(btn, field, v);
}
    ...
    if (e.which == 13) { //.which属性判断按下的是哪个键，回车键的键位序号为13
        switch (arg) {
            case 'order':
                ...
                break;
            case 'product':
                layui.oms_r.search_product()
                break;
            case 'content':
                layui.oms_r.search_content()
                break;

        }
    }
}
search_order_store = function() { layui.oms_r.search_order_store() }
